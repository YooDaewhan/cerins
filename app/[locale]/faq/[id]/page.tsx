...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post =
    (await getPostBySlug("faq", id, locale as LocaleCode)) ??
    (await getPostBySlug("faq", id, "ko"));
  if (!post) return {};
  return {
    title: `${post.title} — CERINS FAQ`,
...
export default async function FaqDetailPage({ params }: Props) {
  ...
  const item =
    (await getPostBySlug("faq", id, code)) ??
    (await getPostBySlug("faq", id, "ko"));
  if (!item) notFound();

  const allPosts = await getPosts("faq", item.locale);
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <nav className="flex items-center gap-2 text-xs text-gray-400 mb-8">
          ...
          <span>/</span>
          <Link href={buildLocalizedPath(code, "/faq")} className="hover:text-(--brand) transition">FAQ</Link>
          <span>/</span>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prev ? (
            <Link
              href={buildLocalizedPath(code, `/faq/${prev.slug}`)}
          ...
          {next ? (
            <Link
              href={buildLocalizedPath(code, `/faq/${next.slug}`)}
        ...
        <div className="mt-6 text-center">
          <Link
            href={buildLocalizedPath(code, "/faq")}
