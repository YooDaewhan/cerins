...
export default function PageHero({ title, subtitle, breadcrumb, image }: PageHeroProps) {
  ...
  return (
    ...
    >
      ...
      />
      {/* Overlay */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#0a1f44]/90 via-[#0a1f44]/75 to-[#0a1f44]/50" />
      {/* Left red accent bar */}
      <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#B4123A]" />

      {/* Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        {breadcrumb && (
          <p className="text-xs text-[#B4123A] tracking-widest uppercase font-bold mb-2 flex items-center gap-2">
            <span className="w-5 h-0.5 bg-[#B4123A] inline-block" />
            {breadcrumb}
          </p>
        )}
